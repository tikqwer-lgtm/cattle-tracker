...
function exportToExcel() {
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      cowRows.push([
        e.cattleId || '', e.nickname || '', e.group || '',
        formatDateForExport(e.birthDate),
        e.lactation !== undefined && e.lactation !== '' ? String(e.lactation) : '',
        formatDateForExport(e.calvingDate), formatDateForExport(e.inseminationDate),
        e.attemptNumber !== undefined ? String(e.attemptNumber) : '1',
        e.bull || '', e.inseminator || '', e.code || '', e.status || '',
        (e.protocol && e.protocol.name) || '',
        formatDateForExport(e.protocol && e.protocol.startDate ? e.protocol.startDate : ''),
        formatDateForExport(e.exitDate), formatDateForExport(e.dryStartDate),
        pdoVal === '—' || pdoVal === '' ? '' : String(pdoVal), e.note || '', e.synced ? 'Да' : 'Нет'
      ]);
    }
    ...
    if (typeof getAllInseminationsFlat === 'function') {
      ...
      for (var j = 0; j < flat.length; j++) {
        var r = flat[j];
        insemRows.push([r.cattleId || '', r.nickname || '', formatDateForExport(r.date), r.attemptNumber !== undefined ? String(r.attemptNumber) : '', r.bull || '', r.inseminator || '', r.daysFromPrevious !== undefined && r.daysFromPrevious !== '—' ? String(r.daysFromPrevious) : '', r.code || '']);
      }
    }
  ...
  for (var k = 0; k < entries.length; k++) {
    ...
    var pdoVal = getPDOForExport(e);
    var row = [
      escapeCsvCell(e.cattleId), escapeCsvCell(e.nickname), escapeCsvCell(e.group || ''),
      escapeCsvCell(formatDateForExport(e.birthDate)), (e.lactation !== undefined && e.lactation !== '' ? String(e.lactation) : ''),
      escapeCsvCell(formatDateForExport(e.calvingDate)), escapeCsvCell(formatDateForExport(e.inseminationDate)),
      (e.attemptNumber !== undefined ? String(e.attemptNumber) : '1'), escapeCsvCell(e.bull), escapeCsvCell(e.inseminator), escapeCsvCell(e.code), escapeCsvCell(e.status),
      ...
      escapeCsvCell(formatDateForExport(e.exitDate)), escapeCsvCell(formatDateForExport(e.dryStartDate)),
      (pdoVal === '—' || pdoVal === '' ? '' : String(pdoVal)), escapeCsvCell(e.note), (e.synced ? 'Да' : 'Нет')
    ];
