...
router.post('/:objectId/protocols', requireAuth, requireRole('admin', 'pro', 'medium', 'lite', 'manager', 'operator'), (req, res) => {
  const objectId = getObjectId(req);
  if (!objectId) return res.status(400).json({ error: 'objectId обязателен' });
  const obj = db.getObjectById(objectId);
  if (!obj) return res.status(404).json({ error: 'Объект не найден' });
...
router.put('/:objectId/protocols/:protocolId', requireAuth, requireRole('admin', 'pro', 'medium', 'lite', 'manager', 'operator'), (req, res) => {
  const objectId = getObjectId(req);
...
router.delete('/:objectId/protocols/:protocolId', requireAuth, requireRole('admin', 'pro', 'medium', 'lite', 'manager', 'operator'), (req, res) => {
  const objectId = getObjectId(req);
  const protocolId = getProtocolId(req);
  if (!objectId || !protocolId) return res.status(400).json({ error: 'objectId и protocolId обязательны' });
  const obj = db.getObjectById(objectId);
