...
т## После обновления кода (git pull) — где видны новые функции

- **Запуск из исходников** (`npm start` из папки `electron/`): приложение грузит страницу из **корня** проекта, поэтому новые изменения в коде видны сразу. Просто перезапустите приложение (закройте окно и снова `npm start`).
- **Установленная версия** (установленный .exe): она собрана из копии кода в `electron/`. Чтобы в установленной программе появились новые функции, нужно **пересобрать** установщик и заново установить приложение:
  ```bash
  cd electron
  npm run dist
  ```
  Затем установите новый файл из `electron/dist/` (например `cattle-tracker-desktop-setup-1.2.4.exe`). Проверка обновлений в меню («Справка → Проверить обновления») подтянет новую версию только если вы выложили новый релиз на GitHub с этим установщиком.
